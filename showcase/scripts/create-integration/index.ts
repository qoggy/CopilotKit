...
`;
  ...
  // 3. Update starter-smoke.yml — add to matrix
  const smokePath = path.join(workflowsDir, "starter-smoke.yml");
  if (fs.existsSync(smokePath)) {
    let smoke = fs.readFileSync(smokePath, "utf-8");
    const slug = args.slug;

    // Match the matrix starter array line, e.g. `starter: [langgraph-python]`
    const matrixRe = /(starter:\s*\[)([^\]]*)(\])/;
    const match = smoke.match(matrixRe);
    if (match) {
      const existing = match[2].split(",").map((s: string) => s.trim());
      if (!existing.includes(slug)) {
        existing.push(slug);
        smoke = smoke.replace(matrixRe, `$1${existing.join(", ")}$3`);
